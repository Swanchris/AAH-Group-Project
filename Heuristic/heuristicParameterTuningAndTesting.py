...
# tune the parameters of the algorithm for a particular instance
        ...
        for mfrac in range(1, 4+1):
            for kind in kinds:
                for i in range(1, 10+1):
                    ...
                    results.append([kind, n, m, X.lowerBound, sol[1], sol[2]])
                    progress += 1
...
# df.loc[df['n'] < 10**5].groupby(["rho", "start", "neighbourhood"]).agg({"cost" : "mean"}).sort_values("cost")
